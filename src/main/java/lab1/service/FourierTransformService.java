...
public class FourierTransformService {
    ...
    public static Pair<List<Complex>, String> BPF(List<Complex> f, boolean inverse) {

        int P = (int) (Math.log(f.size()) / Math.log(2));
        int N = (int) Math.pow(2, P);

        List<Complex> fNew = new ArrayList<>(N);

        for (int i = 0; i < N; i++) {
            fNew.add(new Complex(0));
        }
        for (int i = 0; i < N; i++) {
            int j = reverse(i, P);
            fNew.set(j, f.get(i));
...
//            Complex wm = j.multiply(2. * Math.PI).divide(m).exp();
            Complex wm = polar(1, -2 * Math.PI / m * (inverse ? 1 : -1));
            for (int k = 0; k < N; k += m) {
    ...
    public static Pair<List<Complex>, String> BPFn(List<Complex> f) {
        ...
        for (int i = 0; i < M; i++) {
            ...
            fNew = returnEveryL(fNew, BPF(bpfnStep, false).getFirst(), L, M, i);
        }
